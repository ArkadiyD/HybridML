...
def newick_to_tree(newick, current_labels=dict()):
    ...
    edges, leaves, current_labels, current_node = nested_list_to_tree(
...
def add_node_attributes(tree, distances=True, root=0):
    ...
    for x in tree.nodes:
        if distances:
            ...
            try:
                node_comb = nx.algorithms.shortest_paths.generic.shortest_path_length(
                    tree, root, x)
...
class Input_Set:
    def __init__(self, newick_strings=[], tree_set=None, instance=0, leaves=None, full_leaf_set=True):
        ...
        if full_leaf_set:
            ...
            for t, tree in self.trees.items():
                if t == 0:
                    continue
                self.unique_leaves = tree.leaves.intersection(
                    self.unique_leaves)
    ...
    def CPSBound(self, repeats=1,
        ...
        for i in np.arange(repeats):
            ...
            if progress:
                print(
                    f"Instance {self.instance} {problem_type}: found sequence of length: {len(new)}")
            # COMPLETE PARTIAL SEQUENCE
            new, reduced_trees = sequence_add_roots(new, reduced_trees)
            print(new)
            if progress:
                print(
                    f"Instance {self.instance} {problem_type}: length after completing sequence: {len(new)}")
            ...
            if progress:
                print(
                    f"Instance {self.instance} {problem_type}: best sequence has length: {best_length}")
        ...
        if not self.best_seq_with_lengths or len(new_seq) < len(self.best_seq_with_lengths):
            converted_new_seq = []
            for pair in new_seq:
                converted_new_seq += [(self.labels_reversed[pair[0]],
                                       self.labels_reversed[pair[1]])]
            self.best_seq_with_lengths = converted_new_seq
            self.best_seq_with_lengths_red_trees = red_trees_best
        seq_return = [(self.labels_reversed[x], self.labels_reversed[y])
                      for x, y in new_seq]
    ...
    def CPHeuristic(self, progress=False, reduce_trivial=True, pick_ml=False,
        ...
        # for input in copy_of_inputs.trees:
        #	print('-')
        #	for x,y in copy_of_inputs.trees[input].nw.edges:
        #		print(x,y)
        #	print('-')
        # exit(0)
        CPS = []
        ...
        if pick_ml or pick_ml_triv:
            ...
            df_pred = pd.DataFrame(columns=[
                                   "x", "y", "no_cher_pred", "cher_pred", "ret_cher_pred", "no_ret_cher_pred", "trees_reduced"])
            if progress:
                print(
        ...
        while copy_of_inputs.trees:
            if progress and (pick_ml or pick_ml_triv):
                print(
                    f"Instance {self.instance} {problem_type}: Sequence has length {len(CPS)}")
                print(
                    f"Instance {self.instance} {problem_type}: {len(copy_of_inputs.trees)} trees left.\n")
            if reduce_trivial:
                chosen_cherry, triv_picked = copy_of_inputs.pick_trivial(
                    reducible_pairs)
            ...
            if pick_ml_triv:
                ...
                if trivial_slice.any():       # find trivial cherry
                    triv_cherries = list(
                        features.data.loc[trivial_slice].index)
                    if relabel:
                        triv_cherry_num = np.random.choice(
                            np.arange(len(triv_cherries)))
                        chosen_cherry = triv_cherries[triv_cherry_num]
                    else:
                        chosen_cherry = copy_of_inputs.pick_cherry(
            ...
            if pick_ml:
                ...
                chosen_cherry_prob = prediction.loc[chosen_cherry,
                                                    1] + prediction.loc[chosen_cherry, 2]
                ...
                df_pred.loc[len(df_pred)] = [*chosen_cherry,
                                             *prediction.loc[chosen_cherry], np.nan]
            ...
            if triv_picked and relabel:
                reducible_pairs, merged_cherries = copy_of_inputs.relabel_trivial(
                    *chosen_cherry, reducible_pairs)
                if pick_ml or pick_ml_triv:
                    features.relabel_trivial_features(
                        *chosen_cherry, reducible_pairs, merged_cherries, copy_of_inputs.trees)

            # UPDATE SOME FEATURES BEFORE
            if pick_ml or pick_ml_triv:
                features.update_cherry_features_before(
                    chosen_cherry, reducible_pairs, copy_of_inputs.trees)

            # REDUCE CHOSEN CHERRY FROM FOREST
            new_reduced = copy_of_inputs.reduce_pair_in_all(
                chosen_cherry, reducible_pairs=reducible_pairs)
            picked_nodes = set([pair[0] for pair in CPS])
            print(CPS)
            reducible_pairs = copy_of_inputs.update_reducible_pairs(
                reducible_pairs, new_reduced, picked_nodes)
            # if progress and (pick_ml or pick_ml_triv):
            print(
                f"Instance {self.instance} {problem_type}: {len(reducible_pairs)} reducible pairs left")
            ...
            if pick_ml or pick_ml_triv:
                copy_of_inputs.update_node_comb_length(
                    *chosen_cherry, new_reduced)
                features.update_cherry_features_after(
                    chosen_cherry, reducible_pairs, copy_of_inputs.trees, new_reduced)
    ...
    def pick_cherry(self, triv_cherries, reducible_pairs):
        ...
        for x, y in triv_cherries:
            leaf_x_left, leaf_y_left = self.pick_order(
                x, y, reducible_pairs[x, y], return_cherry=False)
    ...
    def update_reducible_pairs(self, reducible_pairs, new_red_trees, picked_nodes):
        ...
        reducible_pairs_filtered = deepcopy(reducible_pairs)

        # for pair in reducible_pairs:
        #	if pair[1] in picked_nodes:
        #		del reducible_pairs_filtered[pair]

        print('picked', picked_nodes)
        print('r', reducible_pairs)
        # print('rf',reducible_pairs_filtered)
        #reducible_pairs = deepcopy(reducible_pairs_filtered)
        # return reducible_pairs_filtered
        return reducible_pairs
    ...
    def pick_trivial(self, reducible_pairs):
        ...
        if trivial_in_all_cherries:
            chosen_cherry = trivial_in_all_cherries[np.random.choice(
                len(trivial_in_all_cherries))]
            triv_picked = False
        elif trivial_cherries:
            chosen_cherry = trivial_cherries[np.random.choice(
                len(trivial_cherries))]
            triv_picked = True
        else:
            chosen_cherry = None
            triv_picked = False
...
class PhT:
    ...
    def height_of_cherry(self, x, y):
        ...
        for p in self.nw.predecessors(y):
            py = p
        if px == py:
            height = [float(self.nw[px][x]['length']),
                      float(self.nw[py][y]['length'])]
            return height
        return False
    ...
    def clean_node(self, v):
        if self.nw.out_degree(v) == 1 and self.nw.in_degree(v) == 1:
            ...
            if 'length' in self.nw[pv][v] and 'length' in self.nw[v][cv]:
                self.nw[pv][cv]['length'] = self.nw[pv][v]['length'] + \
                    self.nw[v][cv]['length']
...
class PhN:
            ...
            if reduced_trees:
                for i, pair in enumerate(reversed(seq)):
                    if heights:
                        ...
                        current_trees_embedded = current_trees_embedded | reduced_trees[
                            total_len - 1 - i]
                    else:
                        self.add_pair(*pair, red_trees=reduced_trees[total_len - 1 - i],
    ...
    def add_pair(self, x, y, red_trees=set(), current_trees=set(), height=[1, 1]):
        ...
        if len(self.leaves) == 0:
            ...
            self.nw.add_edge(1, 2, no_of_trees=len(
                red_trees), length=height[0])
            self.nw.add_edge(1, 3, no_of_trees=len(
                red_trees), length=height[1])
            self.leaves = {x, y}
            self.labels[x] = 2
            self.labels[y] = 3
            self.leaf_nodes[2] = x
        ...
        self.nw[parent_node_y][node_y]['length'] = length_incoming_y - \
            height_pair_y_real
        ...
        if x not in self.leaves:
            self.nw.add_edge(node_y, self.no_nodes + 1,
                             no_of_trees=len(red_trees), length=height_goal_x)
            self.leaves.add(x)
        ...
        # x is already in the network, so create a reticulate cherry (x,y)
            ...
            if self.nw.in_degree(px) > 1 and length_incoming_x <= height_goal_x:
                self.nw.add_edge(node_y, px, no_of_trees=len(
                    red_trees), length=height_goal_x - length_incoming_x)
            ...
            # create a new reticulation vertex above x to attach the hybrid arc to
                ...
                self.nw.add_edge(node_y, node_x, no_of_trees=len(
                    red_trees), length=height_goal_x - height_pair_x)
                self.nw.add_edge(node_x, self.no_nodes + 1, no_of_trees=no_of_trees_incoming_x + len(red_trees),
                                 length=height_pair_x)
                self.nw[px][node_x]['length'] = length_incoming_x - \
                    height_pair_x
    ...
    def Check_Is_Suppressible(self, v):
        #print( self.nw.out_degree(v),  self.nw.in_degree(v))
        if self.nw.out_degree(v) == 1 and self.nw.in_degree(v) == 1:
            return True
        return False

    def tail_move(self, u, v, s, t):
        if len(set([u, v, s, t])) != 4:
            return False

        check1 = (u, v) in self.nw.edges  # (u,v) is an edge
        check2 = (s, t) in self.nw.edges  # (s,t) is an edge

        if not (check1 and check2):
            return False

        self.nw.remove_edge(u, v)  # delete (u,v)
        check3 = self.Check_Is_Suppressible(u)

        if not check3:
            return False

        self.Suppresss_Node(u)  # suppress u
        self.nw.add_edge(s, u)  # adding u' on the edge (s,t)
        self.nw.add_edge(u, t)  # adding u' on the edge (s,t)
        self.nw.add_edge(u, v)  # adding u',v
